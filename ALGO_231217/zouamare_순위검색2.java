...
public class 순위검색2 {
    ...
    public static int[] solution(String[] info, String[] query) {
        ...
        for(String candidate: info){
            ...
            setting(new String[]{keys[0], "-"}, new String[]{keys[1], "-"}, new String[]{keys[2], "-"}, new String[]{keys[3], "-"}, Integer.parseInt(keys[4]));
    ...
    private static void setting(String[] L, String[] S, String[] C, String[] F, int Value) {
        for(String l : L){
            for(String s : S){
                for(String c : C){
                    for(String f : F){
                        candidatesData.get(l+s+c+f).add(Value);
                    }
                }
            }
        }
